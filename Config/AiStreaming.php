...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | resolve 时从 DB 重建会话历史（StreamHistoryBuilder）下发给 Node 引擎，
    | Node 以服务端历史为唯一上下文（前端上行 messages 仅取当前用户输入）。
    | 工具结果（plan_id/agent_id 所在）随结构化历史传递，不靠模型记忆。
    | 关闭即回滚到旧行为（Node 透传前端纯文本历史）。
    |
    */

    'server_history' => (bool) env('AI_STREAMING_SERVER_HISTORY', true),
